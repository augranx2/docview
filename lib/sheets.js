...
/**
 * Updates every row matching ALL fields in `match` (e.g. one user's row for
 * one document in Document_Access, whose identity is documentId+userEmail
 * rather than a single key column). Returns the number of rows updated.
 */
export async function updateRowsByMatch(tab, match, patch) {
  const data = await callAppsScript({ action: "updateRowsByMatch", tab, match, patch });
  return data.updated;
}
...
export async function createUser(payload) {
  return callAppsScript({ action: "createUser", ...payload });
}

export async function updateUser(payload) {
  return callAppsScript({ action: "updateUser", ...payload });
}

export async function resetUserPassword(username, password) {
  return callAppsScript({ action: "resetPassword", username, password });
}

export async function deleteUser(username) {
  return callAppsScript({ action: "deleteUser", username });
}
...
/**
 * Menyimpan pemberitahuan untuk beberapa pengguna sekaligus.
 *
 * Sengaja tidak pernah melempar galat: pemberitahuan bersifat pelengkap, dan
 * kegagalan menulisnya tidak boleh membatalkan pemberian akses yang sudah
 * berhasil. Tab Notifications juga mungkin belum dibuat pada pemasangan lama.
 */
export async function notify(usernames, { type, title, detail = "" }) {
  const daftar = [...new Set((usernames || []).filter(Boolean))];
  ...
  try {
    await appendRows(
      "Notifications",
      daftar.map((userEmail, i) => ({
        notifId: `${Date.now()}-${i}-${Math.random().toString(36).slice(2, 8)}`,
        userEmail,
        createdAt: now,
        type,
        title,
    ...
    console.error("Gagal menulis notifikasi (diabaikan):", err.message);
  }
}
