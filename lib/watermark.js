...
export async function addControlledWatermark(pdfBuffer, origin, stampInfo = {}) {
  ...
  const alasan = toWinAnsiSafe(stampInfo.reasonLabel || "");
  const lines = [
  ...
  for (const page of pdfDoc.getPages()) {
    ...
    // Alasan pengunduhan dicetak kecil di pojok KIRI bawah, terpisah dari blok
    // identitas di kanan, supaya tidak memperpanjang blok cap dan tetap terbaca.
    if (alasan) {
      const smallSize = Math.max(5.5, Math.min(7.5, width * 0.0105));
      const teks = `Keperluan: ${alasan}`;
      const maxWidth = width - margin * 2 - sealWidth - 12;
      let tampil = teks;
      while (font.widthOfTextAtSize(tampil, smallSize) > maxWidth && tampil.length > 12) {
        tampil = tampil.slice(0, -2);
      }
      if (tampil !== teks) tampil += "...";

      page.drawText(tampil, {
        x: margin,
        y: margin,
        size: smallSize,
        font,
