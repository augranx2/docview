...
// Access levels: Viewer (read-only) < Downloader (read + download) <
// Admin (read + download + upload/manage). Each level includes everything
// below it.
export function canDownload(session) {
  return session.role === "Admin" || session.role === "Downloader";
}

export async function requireDownloadAccess(req, res) {
  const session = await requireSession(req, res);
  if (!session) return null;
  if (!canDownload(session)) {
    res.status(403).json({ error: "Akun Anda tidak memiliki izin download" });
    return null;
  }
  return session;
}
